# Nova forma de programar do airflow 2.0
...
@dag(default_args=default_args, schedule_interval=None, description="ETL de dados do Twitter")
def etl_twitter():
  ...
  def read_data_export_json(retorno):
    with open(base_path + "collected_tweets_2021-03-17-19-22-26.txt") as f:
      tweets = f.readlines()
  ...
  def read_json_export_pandas(retorno):
    arquivos = [file for file in os.listdir(base_path) if file.startwith("tweet_")]
    print(arquivos)
  ...
  check_file = FileSensor(
    task_id="check_file", filepath=f"{base_path}tweet_{ntweets}.json", poke_interval=10
    )
